...
## 36. Значение реестра настроек кванта

Пользовательский интерфейс для управления настройками кванта, описанный ранее, изменяет значение реестра `HKLM\SYSTEM\CurrentControlSet\Control\PriorityControl\Win32PrioritySeparation`. В дополнение к указанию относительной длины квантов потоков (короткий или длинный), это значение реестра также определяет, должны ли потоки в процессе переднего плана получать увеличение кванта (и если да, то на сколько). Это значение состоит из 6 бит, разделенных на три 2-битных поля, как показано на рисунке 6-19.

#### Поля значения реестра Win32PrioritySeparation

1. **Короткий против длинного**: Установка 1 указывает на длинные кванты, в то время как установка 2 указывает на короткие кванты. Установка 0 или 3 означает, что будет использоваться значение по умолчанию (короткий для Windows 2000 Professional и Windows XP, длинный для серверных систем Windows).

2. **Переменная против фиксированной длины**: Установка 1 означает, что квант для процесса переднего плана может изменяться, а установка 2 указывает на то, что значения кванта остаются фиксированными для процессов переднего плана. Установка 0 или 3 означает, что будет использоваться значение по умолчанию (переменная длина для Windows 2000 Professional и Windows XP и фиксированная длина для серверных систем Windows).

3. **Увеличение кванта для переднего плана**: Это поле (хранящееся в переменной ядра `PsPrioritySeparation`) должно иметь значение 0, 1 или 2. Установка 3 является недопустимой и рассматривается как 2. Оно используется как индекс для трехэлементного массива байтов, называемого `PspForegroundQuantum`, чтобы получить квант для потоков в процессе переднего плана. Квант для потоков в фоновых процессах берется из первого элемента этой таблицы квантов.

### Значения кванта

| Шестнадцатеричное | Десятичное | Двоичное   | Интервал | Длина  | PsPrioSep | Квант переднего плана | Квант фонового плана | Общий квант     |
|-------------------|-------------|------------|----------|--------|------------|-----------------------|----------------------|------------------|
| 0x14              | 20          | 0b010100   | Длинный  | Переменный | 12 (62.50мс) | 12 (62.50мс)         | 24 (125.00мс)       |
| 0x15              | 21          | 0b010101   | Длинный  | Переменный | 24 (125.00мс) | 12 (62.50мс)         | 36 (187.50мс)      |
| 0x16              | 22          | 0b010110   | Длинный  | Переменный | 36 (187.50мс) | 12 (62.50мс)         | 48 (250.00мс)      |
| 0x18              | 24          | 0b011000   | Длинный  | Фиксированный | 36 (187.50мс) | 36 (187.50мс)       | 72 (375.00мс)      |
| 0x24              | 36          | 0b100100   | Короткий | Переменный | 6 (31.25мс)  | 6 (31.25мс)          | 12 (62.50мс)       |
| 0x26              | 38          | 0b100110   | Короткий | Переменный | 18 (93.75мс) | 6 (31.25мс)          | 24 (125.00мс)      |

### Увеличение кванта

Когда окно становится активным на клиентской системе, все потоки в процессе, содержащем поток, владеющий активным окном, получают утроенное значение кванта. Таким образом, потоки в процессе переднего плана работают с квантами в 6 тактов, в то время как потоки в других процессах имеют стандартный клиентский квант в 2 такта. Этот механизм обеспечивает то, что при переключении с ресурсоемкого процесса новый процесс переднего плана получит пропорционально больше времени процессора,поскольку его потоки будут иметь более длительный интервал выполнения по сравнению с фоновыми потоками, при условии, что приоритеты потоков в обоих процессах одинаковы.

### Прекращение выполнения

В этой схеме планирования поток с более низким приоритетом может быть прерван, когда поток с более высоким приоритетом становится готовым к выполнению. Это может произойти по нескольким причинам:
- Завершение ожидания потока с более высоким приоритетом (событие, на которое ожидал другой поток, произошло).
- Изменение приоритета потока (приоритет был увеличен или уменьшен).

В любом из этих случаев Windows должна определить, должен ли текущий выполняющийся поток продолжать работу или его следует прервать, чтобы позволить потоку с более высоким приоритетом запуститься. Важно отметить, что потоки, работающие в пользовательском режиме, могут прерывать потоки, работающие в режиме ядра; режим, в котором выполняется поток, не имеет значения. Приоритет потока является определяющим фактором.

Когда поток прерывается, он помещается в начало очереди готовых потоков для приоритета, с которым он выполнялся. Это гарантирует, что когда поток, который прервал, завершит выполнение, прерванный поток сможет завершить свой квант.

### Заключение

Понимание и правильная настройка параметров кванта и планирования потоков в Windows могут существенно повлиять на производительность системы и отзывчивость приложений. Изменение значения реестра `Win32PrioritySeparation` позволяет гибко управлять поведением планировщика, что может быть полезно для оптимизации работы приложений в зависимости от их требований к ресурсам.

Тем не менее, важно помнить, что каждое устройство уникально, и оптимальные настройки могут различаться. Рекомендуется проводить тестирование различных конфигураций, чтобы определить наилучшие значения для вашей системы, обеспечивая баланс между производительностью и отзывчивостью.
